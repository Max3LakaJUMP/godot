	...
	for (List<StringName>::Element *E = animation_names.front(); E; E = E->next()) {
		...
		for (int track_i = 0; track_i < track_count; track_i++){
			...
			if(track_type == Animation::TYPE_VALUE)
				track["update_mode"] = animation->value_track_get_update_mode(track_i);
			Array keys;
...
void Maya::node_to_dict(Node *node, Node *root, Array &nodes, bool child, const Array &properties){
	...
	// if (player){
		...
		// 		track["path"] = animation->track_get_path(track_i);
		// 		track["length"] = keys_count;
		// 		track["api_class"] = animation->track_get_type(track_i);
		// 		track["interpolation_type"] = animation->track_get_interpolation_type(track_i);
		// 		Array keys;
		// 		keys.resize(keys_count);
		// 		for (int key_i = 0; key_i < keys_count; key_i++){
				...
				for (int i = 0; i < bones.size(); i++){
					...
					for (int j = 0; j < polygon->get_bone_count(); j++){
						...
						if (bone_path_real == bone_path){
							...
							Bone2D *bone_2d = Object::cast_to<Bone2D>(node);
							if(bone_2d && bone.has("rest_position") && bone.has("rest_rotation") && bone.has("rest_scale")){
								Transform rest(Basis(Vector3(bone["rest_rotation"]), Vector3(bone["rest_scale"])), bone["rest_position"]);
...
void Maya::dict_to_animation_player(AnimationPlayer *player, const Dictionary &serialized){
	...
	for (int i = 0; i < animations_count; i++){
		...
		if (animation_dict.has("length")){
			float length = animation_dict["length"];
			if(!Math::is_equal_approx(length, animation->get_length()))
		...
		for (int track_i = 0; track_i < track_count; track_i++){
			...
			if(!track.has("type") || !track.has("path"))
				continue;
			if(!track.has("keys"))
				continue;
			...
			Animation::TrackType track_type = (Animation::TrackType)(int) track["type"];
			String track_path = track["path"];
			int track_i_new = -1;
			if (animation->get_track_count() > 0)
				track_i_new = animation->find_track(track_path);
			
			// Remove track only if changed
			if (track_i_new != -1){
				if (animation->track_get_type(track_i_new) != track_type){
					animation->remove_track(track_i_new);
					track_i_new = -1;
				}else{
					int anim_keys_count = animation->track_get_key_count(track_i_new);
					if(anim_keys_count != keys_count){
						animation->remove_track(track_i_new);
						track_i_new = -1;
					}else{
						for (int key_i = 0; key_i < keys_count; key_i++){
							...
							if(!key.has("time"))
								continue;
							float time = key["time"];
							int key_i_new = animation->track_find_key(track_i_new, time, true);
							if (key_i_new == -1){
								animation->remove_track(track_i_new);
								track_i_new = -1;
								break;
			...
			// }
			if (track_i_new == -1){
				track_i_new = animation->add_track(track_type, -1);
			...
			if(track_type != Animation::TYPE_BEZIER && track.has("interpolation_type")){
				Animation::InterpolationType track_interpolation_type = (Animation::InterpolationType) (int) track["interpolation_type"];
				if(track_interpolation_type != animation->track_get_interpolation_type(track_i_new))
					animation->track_set_interpolation_type(track_i_new, track_interpolation_type);
			}
			if(track_type == Animation::TYPE_VALUE && track.has("update_mode")){
				Animation::UpdateMode track_update_mode = (Animation::UpdateMode) (int) track["update_mode"];
				if(track_update_mode != animation->value_track_get_update_mode(track_i_new))
					animation->value_track_set_update_mode(track_i_new, track_update_mode);
			}
			for (int key_i = 0; key_i < keys_count; key_i++){
				Dictionary key = keys[key_i];
				if(!key.has("time") || !key.has("value"))
					continue;
				float time = key["time"];
				Variant value(key["value"]);
				int key_i_new = animation->track_find_key(track_i_new, time, true);
				if (key_i_new == -1){
					if(key.has("transition")){
						animation->track_insert_key(track_i_new, time, key["value"], key["transition"]);
					}else{
						animation->track_insert_key(track_i_new, time, key["value"]);
					}
				}
				else{
					if(value.get_type() == Variant::ARRAY){
						Array v = value;
						Array anim_v = animation->track_get_key_value(track_i_new, key_i_new);
						int value_size = v.size();
						if(value_size == anim_v.size()){
							for (int value_i = 0; value_i < value_size; value_i++){
								if(!Math::is_equal_approx(v[value_i], anim_v[value_i])){
									animation->track_set_key_value(track_i_new, key_i_new, v);
									break;
								}
							}
						}else{
					...
					}else if(!Math::is_equal_approx(value, animation->track_get_key_value(track_i_new, key_i_new))){
						animation->track_set_key_value(track_i_new, key_i_new, value);
					}
					if(key.has("transition")){
						float transition = key["transition"];
						if(!Math::is_equal_approx(transition, animation->track_get_key_transition(track_i_new, key_i_new)))
							animation->track_set_key_transition(track_i_new, key_i_new, transition);
					}
				}
			}
		}
...
Node *Maya::dict_to_node(const Dictionary &serialized, Node *root){
	...
	String type = serialized["api_class"];
...
Maya::Maya() {
	...
	// upb_arena *arena = upb_arena_new();
  	...
  	// serialization_Node* foo = serialization_Node_new(arena);
	// const char greeting[] = "Hello, World!\n";
	// Foo_set_greeting(foo, upb_strview_makez(greeting));
	// print_line(Foo_greeting());
	// serialization_Node node;
	// serialization_Node_set_name(node, "wow");
	// serialization::Vector3 v3;
}
